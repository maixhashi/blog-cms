...
func TestUserUsecase_SignUp(t *testing.T) {
	...
	t.Run("正常系", func(t *testing.T) {
		t.Run("新規ユーザーを登録できる", func(t *testing.T) {
			...
			t.Logf("ユーザー登録: Email=%s", user.Email)
			
			// テスト実行
			userRes, err := uu.SignUp(user)
			...
			if userRes.ID == 0 || userRes.Email != user.Email {
				t.Errorf("SignUp() = %v, want email=%s and ID > 0", userRes, user.Email)
			} else {
			...
			if savedUser.Email != user.Email {
				t.Errorf("SignUp() saved email = %v, want %v", savedUser.Email, user.Email)
			}
...
func TestUserUsecase_Login(t *testing.T) {
	...
	t.Run("正常系", func(t *testing.T) {
		t.Run("有効な認証情報でログインに成功する", func(t *testing.T) {
			...
			t.Logf("ログイン試行: Email=%s", loginUser.Email)
			
			// テスト実行
			tokenString, err := uu.Login(loginUser)

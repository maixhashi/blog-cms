...
var (
	...
	tu             ITaskUsecase // taskUsecase → tu に変更
	testUser       model.User
...
func setupTest() {
	...
	if db != nil {
		...
		tu = NewTaskUsecase(taskRepo, taskValidator) // インスタンス生成部分を修正
	}
...
func TestTaskUsecase_GetAllTasks(t *testing.T) {
	...
	t.Run("正常系", func(t *testing.T) {
		t.Run("正しいユーザーIDのタスクのみを取得する", func(t *testing.T) {
			...
			taskResponses, err := tu.GetAllTasks(testUser.ID) // taskUsecase → tu に変更
...
func TestTaskUsecase_GetTaskById(t *testing.T) {
	...
	t.Run("正常系", func(t *testing.T) {
		t.Run("存在するタスクを正しく取得する", func(t *testing.T) {
			...
			response, err := tu.GetTaskById(testUser.ID, task.ID) // taskUsecase → tu に変更
	...
	t.Run("異常系", func(t *testing.T) {
		t.Run("存在しないIDを指定した場合はエラーを返す", func(t *testing.T) {
			...
			_, err := tu.GetTaskById(testUser.ID, nonExistentTaskID) // taskUsecase → tu に変更
		...
		t.Run("他のユーザーのタスクは取得できない", func(t *testing.T) {
			...
			_, err := tu.GetTaskById(testUser.ID, otherUserTask.ID) // taskUsecase → tu に変更
...
func TestTaskUsecase_CreateTask(t *testing.T) {
	...
	t.Run("正常系", func(t *testing.T) {
		t.Run("新しいタスクを作成できる", func(t *testing.T) {
			...
			t.Logf("タスク作成: Title=%s, UserId=%d", validTask.Title, validTask.UserId)
			
			// テスト実行
			response, err := tu.CreateTask(validTask)
			...
			if response.ID == 0 || response.Title != validTask.Title {
			...
			if savedTask.Title != validTask.Title || savedTask.UserId != testUser.ID {
				t.Errorf("CreateTask() = %v, want title=%s, userId=%d", savedTask, validTask.Title, testUser.ID)
			} else {
	...
	t.Run("異常系", func(t *testing.T) {
		t.Run("バリデーションエラーが発生するタスクは作成できない", func(t *testing.T) {
			...
			t.Logf("無効なタスク作成を試行: Title=%s (長さ: %d)", 
				invalidTask.Title, len(invalidTask.Title))
			
			_, err := tu.CreateTask(invalidTask)
			...
			var count int64
			db.Model(&model.Task{}).Where("title = ?", invalidTask.Title).Count(&count)
			if count > 0 {
...
func TestTaskUsecase_UpdateTask(t *testing.T) {
	...
	t.Run("正常系", func(t *testing.T) {
		t.Run("タスクのタイトルを更新できる", func(t *testing.T) {
			...
			updatedTask := model.Task{
				Title: testutils.GenerateValidTitle() + "2", // 末尾に2を追加して別のタイトルに
			}
			
			t.Logf("タスク更新リクエスト: ID=%d, 新Title=%s", task.ID, updatedTask.Title)
			
			// テスト実行
			response, err := tu.UpdateTask(updatedTask, testUser.ID, task.ID)
			...
			if response.ID != task.ID || response.Title != updatedTask.Title {
				t.Errorf("UpdateTask() = %+v, want id=%d, title=%s", response, task.ID, updatedTask.Title)
			} else {
			...
			if dbTask.Title != updatedTask.Title {
				t.Errorf("UpdateTask() database task title = %v, want %v", dbTask.Title, updatedTask.Title)
			} else {
	...
	t.Run("異常系", func(t *testing.T) {
		t.Run("バリデーションエラーが発生するタスクは更新できない", func(t *testing.T) {
			// 無効な更新 - 長すぎるタイトル
			invalidUpdate := model.Task{
				Title: testutils.GenerateInvalidTitle(),
			}
			
			t.Logf("無効なタイトルでの更新を試行: %s (長さ: %d)", 
				invalidUpdate.Title, len(invalidUpdate.Title))
			
			_, err := tu.UpdateTask(invalidUpdate, testUser.ID, task.ID)
			...
			var dbTask model.Task
			db.First(&dbTask, task.ID)
			if dbTask.Title == invalidUpdate.Title {
		...
		t.Run("存在しないタスクIDでの更新はエラーになる", func(t *testing.T) {
			updateAttempt := model.Task{Title: "Valid"}
